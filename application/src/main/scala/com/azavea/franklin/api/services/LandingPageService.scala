...
class LandingPageService[F[_]: Sync](apiConfig: ApiConfig)(implicit contextShift: ContextShift[F])
  ...
  def conformancePage(acceptHeader: AcceptHeader): F[Either[Unit, JsonOrHtmlOutput]] = {
    ...
    Applicative[F].pure(handleOutput(html, jsonOutput, acceptHeader))
  }

  def landingPage(
      acceptHeader: AcceptHeader
  ): F[Either[Unit, JsonOrHtmlOutput]] = {
    ...
    val result = handleOutput(text.body, landingPage.asJson, acceptHeader)
    Applicative[F].pure(result)
  }

  val routes: HttpRoutes[F] = LandingPageEndpoints.conformanceEndpoint.toRoutes(acceptHeader =>
    conformancePage(acceptHeader)
  ) <+> LandingPageEndpoints.landingPageEndpoint.toRoutes(headers => landingPage(headers))
}
