...
import com.azavea.stac4s.{Bbox, TwoDimBbox, ThreeDimBbox}
...
object SearchParameters {
  ...
  def validate(params: SearchParameters): Option[ValidationError] = {
    ...
    val bboxAndIntersectionFailure = {
      if (params.bbox.isDefined && params.intersects.isDefined)
        Some(ValidationError("Bounding box and intersection are mutually exclusive arguments; please provide one or the other but not both"))
      else
        None
    }
    val bboxFailure = params.bbox.flatMap({
      case TwoDimBbox(xmin, ymin, xmax, ymax) =>
        if (xmax < xmin)
          Some(ValidationError("Invalid bounding box: longitude maximum must not be smaller than longitude minimum"))
        else if (ymax < ymin)
          Some(ValidationError("Invalid bounding box: latitude maximum must not be smaller than latitude minimum"))
        else
          None
      case ThreeDimBbox(xmin, ymin, zmin, xmax, ymax, zmax) =>
        if (xmax < xmin)
          ...
          Some(ValidationError("Invalid bounding box: altitude maximum must not be smaller than altitude minimum"))
        else
          None
    })
  ...
  def getItemsByCollection(collectionId: String, limit: Option[Int] = None, token: Option[String] = None): SearchParameters = SearchParameters(
    None,
    None,
    None,
    List(collectionId),
